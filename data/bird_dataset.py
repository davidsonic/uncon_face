...
class BirdDataset(BaseDataset):
    def initialize(self, opt):
        self.opt = opt
        self.land_root = os.path.join(opt.dataroot,'landmarks_bird')
        self.img_root=os.path.join(opt.dataroot, 'images')


        dirs = os.listdir(self.img_root)
        ...
        for i in range(len(dirs)):
            ...
            ori_name = os.path.join(self.img_root, dirs[i])
            land_name = os.path.join(self.land_root, dirs[i])
